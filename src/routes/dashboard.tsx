...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [{ title: "Dashboard — GovAssist AI" }],
  }),
  component: Dashboard,
});
...
function Dashboard() {
  ...
  return (
    <AppShell>
      <PageHeader
        title={`Welcome back, ${user?.email ?? "Citizen"}`}
        ...
        {QUICK_ACTIONS.map((a) => (
          <Link key={a.to} to={a.to}>
            <Card className="p-5 h-full hover:shadow-elegant hover:-translate-y-0.5 transition-all">
              <div
                className={`grid h-10 w-10 place-items-center rounded-xl ${a.color}`}
              >
        ...
        <Card className="p-6 lg:col-span-2">
          ...
          <div className="mt-4 space-y-3">
            ...
            {applications.map((a: any) => {
              ...
              const svc = getService(a.service_id || a.serviceId);
